...
// V25 Excel template export: exports in Angermund's familiar trip-report layout
function valText(v){
...
function cellContains(cell, needle){
  return valText(cell.value).toLowerCase().includes(String(needle).toLowerCase());
}
function copySheet(src, dest){
  dest.properties = Object.assign({}, src.properties);
  dest.pageSetup = Object.assign({}, src.pageSetup);
  dest.views = JSON.parse(JSON.stringify(src.views || []));
  src.columns.forEach((col, i) => { dest.getColumn(i+1).width = col.width; });
  src.eachRow({ includeEmpty: true }, (row, rowNumber) => {
    const dr = dest.getRow(rowNumber);
    dr.height = row.height;
    row.eachCell({ includeEmpty: true }, (cell, colNumber) => {
      const dc = dr.getCell(colNumber);
      dc.value = cell.value;
      dc.style = JSON.parse(JSON.stringify(cell.style || {}));
      if(cell.numFmt) dc.numFmt = cell.numFmt;
      if(cell.alignment) dc.alignment = JSON.parse(JSON.stringify(cell.alignment));
      if(cell.border) dc.border = JSON.parse(JSON.stringify(cell.border));
      if(cell.fill) dc.fill = JSON.parse(JSON.stringify(cell.fill));
    });
  });
  if(src.model && src.model.merges){
    src.model.merges.forEach(m => { try { dest.mergeCells(m); } catch(e){} });
  }
}
function findLabel(ws, labels){
  let found=null;
  ws.eachRow({ includeEmpty: true }, (row) => {
    row.eachCell({ includeEmpty: true }, (cell) => {
      const txt=valText(cell.value).toLowerCase();
      if(labels.some(l=>txt.includes(l.toLowerCase()))) found = found || cell;
    });
  });
  return found;
}
function writeNear(ws, labels, value){
  if(value===undefined || value===null || value==='') return;
  const c=findLabel(ws, labels);
  if(!c) return;
  // Prefer right cell, then below, then two cells right.
  const options = [
    ws.getCell(c.row, c.col+1),
    ws.getCell(c.row+1, c.col),
    ws.getCell(c.row+1, c.col+1),
    ws.getCell(c.row, c.col+2)
  ];
  const target = options.find(x => !valText(x.value).trim()) || options[0];
  target.value = value;
}
async function fillTripTemplate(ws, trip){
  const driver = trip.driver || '';
  const truck = trip.truck || trip.trailer || '';
  const route = trip.route || [trip.from_place,trip.to_place].filter(Boolean).join(' - ');
  writeNear(ws, ['name of driver','driver name'], driver);
  writeNear(ws, ['registration','truck registration','truck no','truck'], truck);
  writeNear(ws, ['load from','from place','from'], trip.from_place || route.split(' - ')[0] || '');
  writeNear(ws, ['load to','destination','to place','to'], trip.to_place || route.split(' - ')[1] || '');
  writeNear(ws, ['date departure','load date','date'], trip.date || '');
  writeNear(ws, ['odo start','odometer start','start km'], Number(trip.start_km||0));
  writeNear(ws, ['odo end','odometer end','end km'], Number(trip.end_km||0));
  writeNear(ws, ['total kms','total km','km'], Number(trip.km||0));
  writeNear(ws, ['trip income','total income','income'], Number(trip.km||0)*Number(trip.rate_km||0));
  writeNear(ws, ['diesel','fuel expense','fuel slips'], Number(trip.diesel_cost||0));
  writeNear(ws, ['drivers trip money','driver trip money','food money'], Number(trip.food_money||0));
  writeNear(ws, ['profit','profit/loss'], (Number(trip.km||0)*Number(trip.rate_km||0)) - (Number(trip.diesel_cost||0)+Number(trip.food_money||0)+Number(trip.tolls||0)+Number(trip.permits_cost||0)+Number(trip.border_cost||0)+Number(trip.repairs||0)+Number(trip.other_cost||0)));
}
app.get('/api/export/template/trip-report',auth,async(req,res)=>{
  try{
    const templatePath=path.join(__dirname,'templates','trip-report-template.xlsx');
    if(!fs.existsSync(templatePath)) return res.status(404).send('Template file missing: templates/trip-report-template.xlsx');

    const {driver,truck,from,to}=req.query;
    let where=[], vals=[];
    if(driver){ vals.push('%'+driver+'%'); where.push(`lower(driver) like lower($${vals.length})`); }
    if(truck){ vals.push('%'+truck+'%'); where.push(`lower(truck) like lower($${vals.length})`); }
    if(from){ vals.push(from); where.push(`date >= $${vals.length}`); }
    if(to){ vals.push(to); where.push(`date <= $${vals.length}`); }
    const sql=`select * from trips ${where.length?'where '+where.join(' and '):''} order by date desc, id desc limit 20`;
    const trips=(await pool.query(sql,vals)).rows;
    if(!trips.length) return res.status(404).send('No trips found for template export.');
    ...
    outWb.creator='Angermund ERP';
    for(const trip of trips){
      let safeName=(trip.driver||trip.truck||('Trip '+trip.id)).toString().replace(/[\\/*?:[\]]/g,' ').slice(0,25) || ('Trip '+trip.id);
      let name=safeName, counter=1;
      while(outWb.getWorksheet(name)){ counter++; name=(safeName+' '+counter).slice(0,31); }
      const ws=outWb.addWorksheet(name);
      copySheet(templateSheet, ws);
      await fillTripTemplate(ws, trip);
    }

    const data=outWb.addWorksheet('ERP Data');
    data.addRow(['Trip No','Date','Driver','Truck','Route','From','To','KM','Rate/KM','Income','Diesel','Food','Profit','Status']);
    for(const t of trips){
      const income=Number(t.km||0)*Number(t.rate_km||0);
      const cost=Number(t.diesel_cost||0)+Number(t.food_money||0)+Number(t.tolls||0)+Number(t.permits_cost||0)+Number(t.border_cost||0)+Number(t.repairs||0)+Number(t.other_cost||0);
      data.addRow([t.trip_no,t.date,t.driver,t.truck,t.route,t.from_place,t.to_place,Number(t.km||0),Number(t.rate_km||0),income,Number(t.diesel_cost||0),Number(t.food_money||0),income-cost,t.status]);
    }
    data.columns.forEach(c=>c.width=18);

    const buffer=await outWb.xlsx.writeBuffer();
    res.setHeader('Content-Type','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition','attachment; filename=Angermund_Trip_Report_Template_Export.xlsx');
...
app.post('/api/templates/trip-report/upload',auth,admin,upload.single('file'),async(req,res)=>{
  if(!req.file)return res.status(400).json({error:'No template uploaded'});
  const dest=path.join(__dirname,'templates','trip-report-template.xlsx');
  fs.mkdirSync(path.dirname(dest),{recursive:true});
  fs.copyFileSync(req.file.path,dest);
  res.json({ok:true,message:'Trip report template updated'});
});
